...
out_dir <- "results/filtered_plaque_marker_3d_overlays.gene_select/"
...
for (id in sample_ids) {
  ...
  # colors
  genes <- unique(trans_meta$Gene)
  gene_colors <- brewer.pal(length(genes)+1, "Reds")[2:(length(genes)+1)]
  ...
  for (gene in genes) {
    ...
    gene_subset <- trans_meta[trans_meta$Gene == gene,]
    gene_color <- gene_colors[gene]
    
    fig <- fig %>%
      add_trace(
        data = as.data.frame(gene_subset),
        x = ~X_global_um, 
        y = ~Y_global_um, 
        z = ~Z_global_um,
        type = "scatter3d",
