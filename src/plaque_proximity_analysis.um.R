...
out_dir <- "results/plaque_proximity_analysis.um/"
...
analyze_all_plaque_spacing <- function(plaque_boundaries_df) {
  ...
  for (id in unique(plaque_boundaries_df$plaque_id)) {
    plaque <- as.matrix(plaque_boundaries_df[plaque_boundaries_df$plaque_id == id,][,c("X_um","Y_um","Z_um")])
...
sample_min_dists <- lapply(sample_ids, function(id) {
  ...
  cell_coords <- as.matrix(cell_coords[,c("X","Y","Z")])
...
ggplot(sample_min_dists_df,
  ...
  theme_bw()+
  guides(fill="none") +
  labs(x="μm Distance to Plaque", y="Cell Counts")
...
interest_cells <- c("Microglia",
                    "Choroid plexus",
                    "E_Tbr1_Id2",
                    "Oligodendrocyte")

subset <- sample_dist_bin_cell_counts[sample_dist_bin_cell_counts$Cell.Type %in% interest_cells,]

subset$Cell.Type <- factor(as.character(subset$Cell.Type),
                           levels=interest_cells)
